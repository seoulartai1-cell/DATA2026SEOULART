...
`;
  ...
  function addTooltips() {
    const sumTips = [
      ...
      '복귀력·진동·잔상으로 움직임을 디자인해요. ‘원위치 유지/자유’도 골라 보세요.',
      '마우스·소리로 관람자가 작품을 ‘연주’하게 만들어요.',
      '한 번에 분위기를 잡는 프리셋이에요. 적용한 뒤 내 취향으로 다듬어요.',
    ...
    tip('#sel-lens', '구조 렌즈: 에지(윤곽선만 살려 선묘처럼) · 구도(삼분할·시선 무게중심을 드러냄).');
    ...
    tip('#chk-additive', '겹친 빛이 더 밝아지는 발광 효과예요. 성운·네온 느낌이 납니다.');
    tip('#sel-mouse', '관람자의 마우스에 어떻게 반응할지 고릅니다: 밀기/끌기/부풀리기/흩뿌리기.');
    tip('#btn-mic', '마이크로 소리에 반응해요. 잔잔한 곡엔 천천히, 격한 곡엔 폭발처럼 연주됩니다.');
  ...
  const OPTHELP = {
    ...
    valforce: ['값 기반 방향', '각 점의 <b>값</b>(밝기 또는 윤곽 세기)에 따라 가는 방향이 달라져요. 예를 들어 <b>밝을수록 위로</b>, 어두울수록 아래로. 방향은 상하/좌우/중심밖에서 골라요. ‘데이터 값이 높을수록 어디로’를 직접 설계.'],
    ...
    additive: ['발광(빛 번짐)', '겹친 점의 색을 더해(가산 혼합) 밝아지게 합니다. 성운·네온처럼 빛나는 느낌. 어두운 배경에서 강렬해요.'],
    ...
    solid: ['입체 형태 (어디를 원형으로)', '점들을 어떤 3D 면에 배치할지 고릅니다. <b>평면 부조</b>(이미지 그대로 돌출)·<b>구</b>(공에 감싸 ‘원형 보존’)·<b>원기둥</b>(가로를 둥글게 말기)·<b>나선</b>(빙글 감겨 올라가는 띠). 캔버스를 드래그해 돌려 보세요.'],
    focal: ['원근(깊이감)', '카메라와의 거리예요. <b>작을수록 원근이 강해</b>(가까운 점이 과장돼 커짐) 입체감이 극적이고, 클수록 평평하고 차분해요.'],
    dim4: ['4D (초입체 모핑)', '3D에 ‘네 번째 축(W)’을 더해 회전·투영하면, 안과 밖이 뒤집히듯 형태가 <b>스스로 변형(모핑)</b>해요. 4차원을 3차원 그림자로 보는 셈이에요. 데이터가 ‘살아 변형하는 조각’이 됩니다.'],
