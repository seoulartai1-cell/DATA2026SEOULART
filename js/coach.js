...
(function (global) {
  ...
  function offlineQuestions(ctx) {
    ...
    } else if (ctx.kind === 'word') {
      if (ctx.rules) qs.push(`자주 나온 낱말(${ctx.rules})이 가장 큰 글자가 됐어요. ‘무엇을 자주 말했나’가 정말 이 글/작가의 핵심인가요? 한 번만 나왔지만 더 중요한 낱말은 없었나요?`);
      qs.push('불용어(흔한 낱말)로 무엇을 일부러 뺐나요? 그 ‘빼는 선택’ 때문에 사라진 의미는 없을까요? (빈도 분석도 작가의 선택이에요)');
      if (ctx.palette && ctx.palette.length) { const top = ctx.palette[0]; qs.push(`글자 색을 그림의 대표색 ${colorName(top.r, top.g, top.b)}(비율 ${pct(top.ratio)})부터 비율 순으로 입혔네요. 색과 낱말의 짝이 우연인가요, 의도인가요? 색이 낱말의 뜻을 더해 주나요?`); }
      qs.push('모양 틀(원·하트·이니셜·실루엣)이 글의 내용과 어울리나요, 아니면 멋진 형식이 내용을 누르고 있나요?');
      qs.push('이 빈도 분석이 ‘놓친 것’은 무엇일까요? (힌트: 횟수만으로는 맥락·어조·반어를 알 수 없고, 형태소 분석기 없는 어림이라 한 낱말이 쪼개지기도 해요)');
    } else {
